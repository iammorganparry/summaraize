...
export class VideoService {
  ...
  public async getItemFromYoutube(
    ...
    progressStream.on("progress", async (progress) => {
      ...
      const eventToUse = isAudio ? PusherEvents.SummaryProgressAudio : PusherEvents.SummaryProgressVideo;
  ...
  private findSuitableAudioFormat(formats: AdaptiveFormat[]): AdaptiveFormat | undefined {
    return formats.find((f) => f.mimeType.includes("audio/mp4") && f.audioQuality === "AUDIO_QUALITY_MEDIUM");
  }
